...
Databank.prototype = {
    ...
    incr: function(type, id, onCompletion) {
        this.incrBy(type, id, 1, onCompletion);
    },

    decr: function(type, id, onCompletion) {
        this.decrBy(type, id, 1, onCompletion);
    },

    incrBy: function(type, id, n, onCompletion) {
    ...
    decrBy: function(type, id, n, onCompletion) {
        this.incrBy(type, id, -1 * n, onCompletion);
    },

    appendAll: function(type, id, items, onCompletion) {
        this.readAndModify(type,
                           id,
                           items,
                           function(value) { return value.concat(items); },
                           function(err, result) {
                               onCompletion(err);
                           });
    },

    prependAll: function(type, id, items, onCompletion) {
        this.readAndModify(type,
                           id,
                           items,
                           function(value) { return items.concat(value); },
                           function(err, result) {
                               onCompletion(err);
                           });
    },

    append: function(type, id, item, onCompletion) {
        this.appendAll(type, id, [item], onCompletion);
    },

    prepend: function(type, id, item, onCompletion) {
        this.prependAll(type, id, [item], onCompletion);
    },
    ...
    remove: function(type, id, item, onCompletion) {
        this.removeAll(type, id, [item], onCompletion);
    },

    removeAll: function(type, id, items, onCompletion) {
        var bank = this;
        ...
        Step(
            ...
            function(err, value) {
                ...
                if (err) throw err;
                if (!Array.isArray(value)) {
                    throw new WrongTypeError(type, id, "array");
                }
                ...
                bank.update(type, id, value, this);
            },
            function(err, value) {
                if (err) throw err;
                this(null);
    ...
    length: function(type, id, onCompletion) {
        bank = this;
        bank.read(type, id, function(err, value) {
            if (err) {
                onCompletion(err);
            } else if (!Array.isArray(value)) {
                onCompletion(new WrongTypeError(type, id, "array"));
            } else {
                onCompletion(null, value.length);
            }
        });
    },

    truncate: function(type, id, length, onCompletion) {
        bank = this;
        bank.read(type, id, function(err, value) {
            var newValue = null;
            if (err) {
                ...
                bank.update(type, id, newValue, function(err, results) {
                    if (err) {
                        onCompletion(err);
                    } else {
                        onCompletion(null);
