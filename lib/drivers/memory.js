...
MemoryDatabank.prototype.incrBy = function(type, id, n, onCompletion) {
    var bank = this;
    if (!bank.connected) {
        onCompletion(new NotConnectedError());
        return;
...
MemoryDatabank.prototype.remove = function(type, id, item, onCompletion) {
    ...
    setImmediate(function() {
        ...
        if (!(id in bank.types[type])) {
            ...
            // Only remove if it's not already there
            if (i !== -1) {
...
MemoryDatabank.prototype.length = function(type, id, onCompletion) {
    var bank = this;
    if (!bank.connected) {
        onCompletion(new NotConnectedError());
        return;
    ...
    setImmediate(function() {
        ...
        if (!(id in bank.types[type])) {
            onCompletion(new NoSuchThingError(type, id), null);
        } else if (!Array.isArray(bank.types[type][id])) {
            onCompletion(new WrongTypeError(type, id, "array"));
        } else {
            onCompletion(null, bank.types[type][id].length);
        }
    });
};
